...
#[derive(Clone, Copy, Debug, PartialEq)]
pub(crate) struct UiReadability {
...
#[derive(Clone, Copy, Debug, PartialEq)]
pub(crate) struct PlayerNoticeStyle {
...
pub(crate) const REFERENCE_WIDTH: f32 = 1920.0;
...
#[derive(Clone, Copy, Debug, PartialEq)]
pub(crate) struct ResponsiveLimits {
    pub min_scale: f32,
    pub max_scale: f32,
...
#[derive(Clone, Copy, Debug, PartialEq)]
pub(crate) struct PanelConstraints {
    pub min_width: f32,
    pub max_width: f32,
    pub min_height: f32,
    pub max_height: f32,
...
#[cfg(test)]
mod tests {
    use super::{ComponentState, TextRole, UiTheme};
    ...
    fn slot_distinguishes_every_gameplay_state() {
        // Every state must be visually distinct so players can tell at a
        // glance whether a slot is empty, disabled, alerting, etc.
        let slot = UiTheme::VOXELVERSE.slot;
        let states = [
    ...
    fn viewport_scaling_clamps_in_sensible_range() {
        let theme = UiTheme::VOXELVERSE;
        let baseline = theme.auto_scale(super::UiViewport::new(1920.0, 1080.0));
        assert!((baseline - 1.0).abs() < f32::EPSILON);
        // 720p stays above the min auto scale.
        assert!(theme.auto_scale(super::UiViewport::new(1280.0, 720.0)) >= 0.75);
        // 4K stays below the max auto scale.
        assert!(theme.auto_scale(super::UiViewport::new(3840.0, 2160.0)) <= 1.60);
    }

    #[test]
    fn effective_scale_combines_auto_and_user_zoom() {
        let theme = UiTheme::VOXELVERSE;
        let vp = super::UiViewport::new(1920.0, 1080.0);
        assert!((theme.effective_scale(vp, super::UserZoom::Normal) - 1.0).abs() < 1e-4);
        assert!((theme.effective_scale(vp, super::UserZoom::XXLarge) - 1.5).abs() < 1e-4);
        // User zoom can't push past max_scale on a 4K screen.
        let big = super::UiViewport::new(3840.0, 2160.0);
        assert!(theme.effective_scale(big, super::UserZoom::XXLarge) <= 2.20 + 1e-4);
    }

    #[test]
    fn user_zoom_presets_cover_required_steps() {
        let factors: Vec<f32> = super::UserZoom::ALL.iter().map(|z| z.factor()).collect();
    ...
    fn adaptive_grid_adapts_columns_to_available_width() {
        let grid = super::AdaptiveGrid {
            min_columns: 5,
            preferred_columns: 8,
            max_columns: 12,
        };
        // Plenty of room → max columns capped.
        assert_eq!(grid.columns_for(2000.0, 58.0, 8.0), 12);
        // Just enough for ~8 slots.
        assert_eq!(grid.columns_for(8.0 * 58.0 + 7.0 * 8.0, 58.0, 8.0), 8);
        // Too narrow → falls back to min_columns (caller must scroll).
    ...
    fn anchor_origin_keeps_blocks_inside_viewport() {
        let theme = UiTheme::VOXELVERSE;
        let vp = super::UiViewport::new(1920.0, 1080.0);
        // BottomCenter places block centered horizontally, bottom-aligned.
        let (x, y) = theme.anchor_origin(vp, super::UiAnchor::BottomCenter, (400.0, 80.0), 30.0);
        assert!((x - (1920.0 - 400.0) * 0.5).abs() < 1.0);
        assert!((y - (1080.0 - 80.0 - 30.0)).abs() < 1.0);
        // Center places block in the middle.
        let (x, y) = theme.anchor_origin(vp, super::UiAnchor::Center, (600.0, 400.0), 0.0);
    ...
    fn motion_durations_stay_short_enough_to_not_delay_gameplay() {
        ...
        // Interactive feedback must land in well under a quarter second.
