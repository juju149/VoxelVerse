...
impl Controller {
    ...
    pub fn update_player(&mut self, player: &mut Player, planet: &PlanetData, physics: &Physics, dt: f32) {
        let mut input = Vec3::ZERO;
        if self.keys[0] { input.z -= 1.0; }
        if self.keys[1] { input.x -= 1.0; }
        if self.keys[2] { input.z += 1.0; }
        if self.keys[3] { input.x += 1.0; }
        let jump = self.keys[4];

        let rotation_delta = if self.first_person { self.mouse_delta } else { (0.0, 0.0) };
        player.update(dt, planet, physics, input, jump, rotation_delta, self.fly_mode, self.sprint);
    ...
    pub fn get_matrix(&self, player: &Player, physics: &Physics, width: f32, height: f32, render_cfg: &vv_config::RenderConfig) -> Mat4 {
        let fov = if self.first_person { render_cfg.fov_first_person_deg } else { render_cfg.fov_orbit_deg };
            ...
            WindowEvent::KeyboardInput { event, .. } => {
                ...
                match event.physical_key {
                    ...
                    PhysicalKey::Code(KeyCode::KeyP) if pressed => {
                        if player.debug_mode { self.is_wireframe = !self.is_wireframe; }
                        return true;
                    }
                    ...
                    PhysicalKey::Code(KeyCode::Quote) if pressed => {
                        if player.debug_mode { self.freeze_culling = !self.freeze_culling; }
                        return true;
                    }
