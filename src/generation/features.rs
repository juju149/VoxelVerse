...
use crate::content::{
    CompiledProceduralPlanet, CompiledTreeShapeKind, CompiledVegetation, CompiledVisualDetail,
    ProceduralRegistry,
};
...
impl TreeShape {
    pub fn compute(
        ...
        .max(1) as f32;
        let canopy_radius =
            range_pick(scale_range(veg.canopy_radius, scale), hash4(face, pu, pv, 34)) as f32;
        ...
        let (branches, lobes) = match veg.shape_kind {
            CompiledTreeShapeKind::Conical => {
                build_conical(veg, face, pu, pv, plant_height, height, canopy_radius, &pivot, scale)
            }
    ...
    pub fn stamp(&self, emit: &mut dyn FnMut(i32, i32, i32, VoxelId)) {
        ...
        for (bi, branch) in self.branches.iter().enumerate() {
            ...
            if let Some(tip) = branch.tip {
                stamp_lobe(tip, self.leaves_block, self.canopy_density, self.canopy_seed, emit);
            }
        }

        // Canopy: union of jittered lobes.
        for lobe in &self.lobes {
            stamp_lobe(*lobe, self.leaves_block, self.canopy_density, self.canopy_seed, emit);
        }
    }
...
fn build_broad_leaf(
    ...
    let branch_len_max =
        range_pick(scale_range(veg.branch_length, scale), hash4(face, pu, pv, 37)).max(1) as f32;
    ...
    if branch_count > 0 && height >= 3 {
        ...
        for i in 0..branch_count {
            ...
            let raw_len = (1.5 + hash01_u32(h, 1) * (branch_len_max + 1.0))
                .min(branch_len_max + 2.0);
    ...
    if canopy_radius > 0.0 {
        ...
        for i in 0..n_lobes {
            ...
            let center = canopy_anchor
                + Vec3::new(theta.cos() * r_off, y_off, theta.sin() * r_off);
...
fn build_flat_top(
    ...
    let branch_len_max =
        range_pick(scale_range(veg.branch_length, scale), hash4(face, pu, pv, 37)).max(2) as f32;
    ...
    let halo = CanopyLobe {
        center: Vec3::new(
            pivot.x,
            plant_height as f32 + height as f32 + 0.1,
            pivot.z,
        ),
