...
///
/// Handles text input, command parsing, and visual animation state.
/// Commands can read or write runtime-tunable player parameters.
pub struct Console {
    pub is_open: bool,
    pub input_buffer: String,
    pub history: Vec<(String, [f32; 3])>,
    /// Fraction in `[0, 1]` used for the slide-in animation.
...
impl Console {
    ...
    fn handle_property_command(&mut self, parts: Vec<&str>, name: &str, property: &mut f32) {
        ...
        match parts[1] {
            "get" => self.log(&format!("{} = {:.2}", name, property), [0.0, 1.0, 0.0]),
            "set" => {
                if parts.len() < 3 {
