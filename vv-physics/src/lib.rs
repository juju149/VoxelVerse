...
impl Physics {
    ...
    pub fn is_solid(&self, pos: Vec3, planet: &PlanetData) -> bool {
        ...
        if local.x < margin && id.u > 0 {
            if !planet.exists(BlockId { u: id.u - 1, ..id }) { return false; }
        } else if local.x > 1.0 - margin && id.u < res - 1 {
            if !planet.exists(BlockId { u: id.u + 1, ..id }) { return false; }
        }
        if local.y < margin && id.v > 0 {
            if !planet.exists(BlockId { v: id.v - 1, ..id }) { return false; }
        } else if local.y > 1.0 - margin && id.v < res - 1 {
            if !planet.exists(BlockId { v: id.v + 1, ..id }) { return false; }
        }
        if local.z < margin && id.layer > 0 {
            if !planet.exists(BlockId { layer: id.layer - 1, ..id }) { return false; }
    ...
    fn get_grid_axes(up: Vec3, pos: Vec3) -> (Vec3, Vec3) {
        let abs_p = pos.abs();
        let rigid_axis = if abs_p.y >= abs_p.x && abs_p.y >= abs_p.z { Vec3::X }
                         else { Vec3::Y };
    ...
    pub fn check_collision(&self, pos: Vec3, planet: &PlanetData) -> bool {
        ...
        let checks = [pos, pos + up * 0.9, pos + up * self.eye_height, pos + up * self.player_height];
        let (rd, fd) = Self::get_grid_axes(up, pos);
        let right = rd * self.player_radius;
        let fwd   = fd * self.player_radius;
        for cp in checks {
            if self.is_solid(cp, planet)         { return true; }
            if self.is_solid(cp + right, planet) { return true; }
            if self.is_solid(cp - right, planet) { return true; }
            if self.is_solid(cp + fwd,   planet) { return true; }
            if self.is_solid(cp - fwd,   planet) { return true; }
        }
        false
    }
    ...
    pub fn solve_movement(
        &self,
        start_pos: Vec3, velocity: Vec3, dt: f32,
        planet: &PlanetData, flying: bool,
    ) -> (Vec3, Vec3, bool) {
        if flying {
