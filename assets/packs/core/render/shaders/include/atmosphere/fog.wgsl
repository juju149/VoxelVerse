...
fn vv_aerial_fog(color: vec3<f32>, world_pos: vec3<f32>) -> vec3<f32> {
    ...
    let scatter = clamp(vv_forward_scatter(view_dir) * 0.12, 0.0, 0.08);

    // Blue-grey atmospheric haze, deliberately not white.
    let night_haze = vec3<f32>(0.038, 0.046, 0.070);
    let day_haze = global.sky_horizon.rgb * vec3<f32>(0.16, 0.20, 0.26);
    ...
    fog_col = min(fog_col, vec3<f32>(0.24, 0.30, 0.38));
    ...
    let luma_cap = min(1.0, (source_luma + 0.035) / fog_luma);
    fog_col *= luma_cap;
