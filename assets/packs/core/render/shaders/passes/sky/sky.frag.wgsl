...
fn fs_main(in: FullscreenVertexOut) -> @location(0) vec4<f32> {
    ...
    let sun_col = mix(vec3<f32>(1.05, 0.42, 0.08), vec3<f32>(1.25, 1.15, 0.92), clamp(sun_elev * 2.8 + 0.25, 0.0, 1.0));
    ...
    sky += vv_disc_glow(in.uv, moon_dir, 0.0018, 18.0) * vec3<f32>(0.42, 0.46, 0.60) * moon_vis;
