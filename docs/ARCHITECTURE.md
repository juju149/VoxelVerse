...
- `vv-content-schema`: raw gameplay/content `.ron` structs only. It does not read files or assign runtime IDs; render pipelines are not content schema.
...
- `vv-render`: owns render profiles, shader loading, pass order, pipeline descriptors, bind group layouts and global uniforms. WGSL source lives in packs at stable paths under `render/shaders`, but `.ron` render graphs/profiles/techniques are intentionally forbidden.
