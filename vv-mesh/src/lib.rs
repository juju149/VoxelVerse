...
use vv_world_runtime::{PlanetData, ChunkMods};
...
impl MeshGen {
    ...
    fn add_voxel(id: BlockId, data: &PlanetData, verts: &mut Vec<Vertex>, inds: &mut Vec<u32>, idx: &mut u32) {
        ...
        let check = |_face: u8, d_layer: i32, d_u: i32, d_v: i32| -> bool {
            ...
            if l >= 0 && u >= 0 && u < res as i32 && v >= 0 && v < res as i32 {
                return data.exists(BlockId { face: id.face, layer: l as u32, u: u as u32, v: v as u32 });
            }
            l < 0
        };
        ...
        for i in 1..=8 {
            if check(id.face, i, 0, 0) { light_val = 0.15; break; }
        }
        let natural_h = data.terrain.get_height(id.face, id.u, id.v);
        if id.layer >= natural_h { light_val = 1.0; }

        let is_core  = data.has_core && id.layer < data.core_protection_layers;
        let is_grass = id.layer == natural_h;
        let mut base_color = if is_core  { [0.2, 0.2, 0.2] }
                             else if is_grass { [0.1, 0.7, 0.1] }
                             else { [0.6, 0.4, 0.2] };
        ...
        let i_bl = p(0,0,0); let i_br = p(1,0,0);
        let i_tl = p(0,1,0); let i_tr = p(1,1,0);
        let o_bl = p(0,0,1); let o_br = p(1,0,1);
        let o_tl = p(0,1,1); let o_tr = p(1,1,1);

        let apply = |ao: f32| -> [f32; 3] {
        ...
        if !has_top {
            ...
            Self::quad(verts, inds, idx, [o_bl, o_br, o_tr, o_tl],
                [apply(ao_bl), apply(ao_br), apply(ao_tr), apply(ao_tl)], true);
        }
        if !has_btm {
            let c = apply(0.4);
        ...
        if !has_front { Self::quad(verts, inds, idx, [i_bl, i_br, o_br, o_bl], sc, false); }
        if !has_back  { Self::quad(verts, inds, idx, [o_tl, o_tr, i_tr, i_tl], sc, false); }
        if !has_left  { Self::quad(verts, inds, idx, [i_tl, i_bl, o_bl, o_tl], sc, false); }
        if !has_right { Self::quad(verts, inds, idx, [i_br, i_tr, o_tr, o_br], sc, false); }
    }
    ...
    fn calculate_ao(side1: bool, side2: bool, corner: bool) -> f32 {
        ...
        match occ { 0 => 1.0, 1 => 0.8, 2 => 0.6, _ => 0.4 }
    }

    fn quad(
        verts: &mut Vec<Vertex>, inds: &mut Vec<u32>, idx: &mut u32,
        pos: [Vec3; 4], colors: [[f32; 3]; 4], force_radial: bool,
    ) {
        let normal = if force_radial {
            ((pos[0] + pos[1] + pos[2] + pos[3]) * 0.25).normalize().to_array()
        } else {
            (pos[1] - pos[0]).cross(pos[2] - pos[0]).normalize().to_array()
        };
        let base = *idx;
        for (p, c) in pos.iter().zip(colors.iter()) {
            verts.push(Vertex { pos: p.to_array(), color: *c, normal });
        }
    ...
    pub fn generate_lod_mesh(key: LodKey, data: &PlanetData, grid_res: u32) -> (Vec<Vertex>, Vec<u32>) {
        let mut verts = Vec::new();
        let mut inds  = Vec::new();
        ...
        for vy in 0..=grid_res {
            for ux in 0..=grid_res {
                ...
                let is_core  = data.has_core && h < data.core_protection_layers;
                let is_steep = slope < 0.85;
                let color = if is_core   { [0.2, 0.22, 0.25] }
                             else if is_steep { [0.075, 0.6, 0.075] }
                             else             { [0.1, 0.8, 0.1] };
    ...
    pub fn generate_collision_debug(player_pos: Vec3, planet: &PlanetData) -> (Vec<Vertex>, Vec<u32>) {
        ...
        if let Some((center_id, _)) = CoordSystem::get_local_coords(player_pos, res) {
            ...
            for l in sl..=el {
                for v in sv..=ev {
                    for u in su..=eu {
                        let id = BlockId { face: center_id.face, layer: l as u32, u: u as u32, v: v as u32 };
                        if !planet.exists(id) { continue; }
                        let gp = |uu, vv, ll| CoordSystem::get_vertex_pos(id.face, id.u + uu, id.v + vv, id.layer + ll, res);
                        let c000=gp(0,0,0); let c100=gp(1,0,0);
                        let c010=gp(0,1,0); let c110=gp(1,1,0);
                        let c001=gp(0,0,1); let c101=gp(1,0,1);
                        let c011=gp(0,1,1); let c111=gp(1,1,1);
                        let center = (c000+c100+c010+c110+c001+c101+c011+c111) * 0.125;
                        let sh = 0.90f32;
                        let vi = |p: Vec3| Vertex { pos: (center + (p - center) * sh).to_array(), color, normal };
                        let corners = [vi(c000),vi(c100),vi(c110),vi(c010),vi(c001),vi(c101),vi(c111),vi(c011)];
                        for c in &corners { verts.push(*c); }
                        let b = idx;
                        let lines = [(0,1),(1,2),(2,3),(3,0),(4,5),(5,6),(6,7),(7,4),(0,4),(1,5),(2,6),(3,7)];
                        for (s, e) in lines { inds.push(b+s); inds.push(b+e); }
                        idx += 8;
                    }
                }
            }
    ...
    pub fn generate_cylinder(radius: f32, height: f32, segments: u32) -> (Vec<Vertex>, Vec<u32>) {
        ...
        for i in 0..=segments {
            ...
            verts.push(Vertex { pos: [x, 0.0,    z], color, normal: n });
            verts.push(Vertex { pos: [x, height, z], color, normal: n });
        }
        for i in 0..segments {
            let b1 = i * 2; let t1 = b1 + 1;
            let b2 = b1 + 2; let t2 = b1 + 3;
        ...
        for i in 0..=segments {
            let theta = (i as f32 / segments as f32) * std::f32::consts::TAU;
            verts.push(Vertex { pos: [theta.cos()*radius, height, theta.sin()*radius], color, normal: [0.0,1.0,0.0] });
        }
        for i in 0..segments {
            inds.push(ci); inds.push(ci + 1 + i); inds.push(ci + 2 + i);
        }
        (verts, inds)
    }
    ...
    pub fn generate_sphere_guide(radius: f32, segments: u32) -> (Vec<Vertex>, Vec<u32>) {
        ...
        for y in 0..=segments {
            for x in 0..=segments {
                ...
                verts.push(Vertex { pos: [xp*radius, yp*radius, zp*radius], color, normal: [xp,yp,zp] });
            }
        }
        for y in 0..segments {
            for x in 0..segments {
                let i = y * (segments + 1) + x;
                inds.extend_from_slice(&[i, i+segments+1, i+segments+2, i+segments+2, i+1, i]);
            }
        }
        (verts, inds)
    }
